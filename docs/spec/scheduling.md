...
- **RTOS-PORT-004:** A block, sleep, or exit handoff linearizes the source state
  transition, source resume-generation check, ready-target ownership transfer,
  and pending-intent commit in one scheduler critical section. It has exactly
  two valid outcomes: either one committed pending intent uniquely owns the
  detached target, or the source has resumed and the operation returns
  `NoSwitch` without removing, duplicating, or reordering that ready target.
  Thread-mode handoffs use a sequence-bearing switch intent that captures source
  and target identity generations plus the source resume generation. The trap
  consumes a committed pending intent at most once. The ticket-creation model
  proves the prepare-or-observe-resume decision; the ticket-lifecycle model
  separately proves commit, cancellation, identity reuse, and consumption. The
  earlier state-based recovery remains as a compatibility guard until
  on-silicon ticket parity is established.
